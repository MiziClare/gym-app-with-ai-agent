...
const names: Record<string, string> = {
  ...
  equipment: 'Equipment', equipmentReservations: 'Equipment Bookings', posts: 'Community Posts', profile: 'My Account',
}
...
async function remove(item: Row) {
  try {
    await ElMessageBox.confirm('Delete this record?', 'Please confirm')
    await api.delete(`/admin/${module.value}/${item.id}`)
  ...
  <div class="admin-page">
    ...
    <section v-if="module !== 'overview' && !loading" class="admin-panel table-wrap">
      <table v-if="rows.length">
        <thead><tr><th v-for="key in Object.keys(rows[0])" :key="key">{{ label(key) }}</th><th v-if="['notices','equipment','posts','courses','members','coaches'].includes(module)">Actions</th></tr></thead>
        <tbody><tr v-for="item in rows" :key="item.id || item.username"><td v-for="key in Object.keys(rows[0])" :key="key">{{ show(item[key]) }}</td><td v-if="['notices','equipment','posts','courses','members','coaches'].includes(module)" class="table-actions"><template v-if="module === 'equipment'"><button type="button" @click="equipmentStatus(item, 'AVAILABLE')">Available</button><button type="button" @click="equipmentStatus(item, 'MAINTENANCE')">Maintenance</button></template><button v-else-if="module === 'courses'" type="button" @click="setActive('courses', item)">{{ item.active ? 'Disable' : 'Enable' }}</button><button v-else-if="['members','coaches'].includes(module)" type="button" @click="setActive('users', item)">{{ item.active ? 'Disable' : 'Enable' }}</button><button v-else type="button" @click="remove(item)">Delete</button></td></tr></tbody>
      </table>
